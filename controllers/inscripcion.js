...
inscripcionRouter.post('/', async (request, response) => {
    ...
    const materiaId = request.body.materiaId || request.body.id || request.body._id;
    if (request.rol !== 'estudiante') {
        return response.status(403).json({ error: 'Solo los estudiantes pueden inscribir' });
    }
    ...
    return response.status(400).json({ error: 'Ya estás inscrito en esta materia' });
        ...
        return response.status(400).json({ error: 'ID de materia no válido o error de servidor' });
    }
});
